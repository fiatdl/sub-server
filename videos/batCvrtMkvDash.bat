...
REM ffmpeg -re -i "%~1.mkv" -map 0 -map 0 ^
REM -c:a copy ^
REM -c:v copy ^
...
REM -use_timeline 1 -single_file 1 -use_template 1 -adaptation_sets "id=0,streams=v id=1,streams=a" ^
REM -f dash "%~1/init.mpd"

ffmpeg -re -i "%~1.mkv" -map 0 -map 0 -preset ultrafast ^
-c:a copy ^
-c:v copy ^
